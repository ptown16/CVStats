...
public class LeaderboardSetValueColor extends BaseCommand {

    public LeaderboardSetValueColor() {
        setPermission("cvstats.leaderboards.setvaluecolor");
        setHelpValue("/cvstats leaderboards <leaderboard> setvaluecolor <#ffffff>", "Set the key to sort a leaderboard on");
    }
    ...
    protected boolean runCommand(CommandSender sender, String[] args, List<Object> passedArgs) {
        ...
        leaderboard.valueColor = color;
        CVStats.getInstance().saveLeaderboardManager();
        leaderboard.reload();
        return sendSuccess(sender, "Set value color of leaderboard " + leaderboard.id + " to be " + leaderboard.key + "!");
    }
